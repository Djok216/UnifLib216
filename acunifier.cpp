...
ACUnifier::ACUnifier(FastTerm t1, FastTerm t2) : t1(t1), t2(t2) {}

void ACUnifier::delSameCoeffs(map<FastTerm, int> &l, map<FastTerm, int> &r) {
  if (l.size() > r.size()) {
    l.swap(r);
  }
...
vector<int> ACUnifier::fromMapToVector(const map<FastTerm, int> &M) {
  vector<int> ans;
  ans.reserve(M.size());
  for (auto it : M) {
...
FastTerm ACUnifier::createFuncWithSameVar(int cnt, FastTerm var, FastFunc f, FastTerm unityElement) {
...
vector<FastSubst> ACUnifier::solveAC(UnifEq ueq) {
...
vector<FastSubst> ACUnifier::solve() {
  UnifEqSystem ues(t1, t2);
  return solve(ues);
}

vector<FastSubst> ACUnifier::solve(UnifEqSystem ues) {
  auto initUes = ues;
  vector<FastSubst> minSubstSet;
